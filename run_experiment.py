...
# Update the models to include orca-mini with orca-mini as the default
current_models = {
    'medium': 'Phi-3-medium-128k-instruct',
    'mini': 'Phi-3-mini-128k-instruct',
    'orca-mini': 'Orca-mini-128k-instruct'  # Add orca-mini model here
}
...
def main():
    ...
    parser.add_argument('--data_file', type=str, default='java_test_clone.jsonl', help='The data file containing code samples')
    parser.add_argument('--result_dir', type=str, default='model_result', help='The location to save the results')
    parser.add_argument('--model', type=str, default='orca-mini', help='The model to use')  # Default set to orca-mini

    args = parser.parse_args()
    model = args.model  # Set the model to the provided argument or default to 'orca-mini'
    
    # Determine the model class
    if args.model_class == 'offline':
        model_cls = OfflineRequest
        model = current_models.get(model, current_models['orca-mini'])  # Default to orca-mini if model not found
